...
class App:
    def __init__(self, root):
        ...
        tk.Button(self.frame_entrada, text="Adicionar Recurso", command=self.adicionar_recurso).grid(row=3, column=0, columnspan=2, pady=(15, 15))
        ...
        tk.Button(self.frame_entrada, text="Adicionar Processo", command=self.adicionar_processo).grid(row=7, column=0, columnspan=2, pady=(15, 15))
        ...
        tk.Button(self.frame_entrada, text="Remover Processo", command=self.remover_processo).grid(row=9, column=0, columnspan=2, pady=(15, 15))
        ...
        tk.Button(self.frame_entrada, text="Iniciar Simulação", command=self.iniciar_simulacao).grid(row=11, column=0, columnspan=2, pady=(15, 15))
    ...
    def adicionar_recurso(self):
        ...
        if nome and id_recurso.isdigit() and quantidade.isdigit():
            self.recursos.append(Recurso(nome, int(id_recurso), int(quantidade)))
    ...
    def adicionar_processo(self):
        ...
        if id_processo.isdigit() and intervalo.isdigit() and tempo_utilizacao.isdigit():
            processo = Processo(
                ...
                intervalo_solicitacao=int(intervalo),
    ...
    def remover_processo(self):
        ...
        if processo:
            processo.parar()  # Sinaliza a thread para encerrar
    ...
    def iniciar_simulacao(self):
        ...
        self.so = SistemaOperacional(self.recursos, self.processos, int(intervalo_verificacao), self.atualizar_log)
        self.so.start()
    ...
    def atualizar_matrizes(self):
        ...
        for r in self.recursos:
            self.txt_recursos_existentes.insert(tk.END, f"{r.nome}: {r.quantidade}\n")
        
        # Recursos Disponíveis
        self.txt_recursos_disponiveis.delete("1.0", tk.END)
        for r in self.recursos:
            self.txt_recursos_disponiveis.insert(tk.END, f"{r.nome}: {r.disponivel}\n")
        
        # Processos com Recursos Alocados
        self.txt_processos_alocados.delete("1.0", tk.END)
        for p in self.processos:
            if p.recurso_atual:
                self.txt_processos_alocados.insert(tk.END, f"Processo {p.id}: {p.recurso_atual.nome}\n")
            else:
                self.txt_processos_alocados.insert(tk.END, f"Processo {p.id}: -\n")
        
        # Processos Bloqueados – Recursos Aguardados
        self.txt_processos_bloqueados.delete("1.0", tk.END)
        for p in self.processos:
            if p.status == "Bloqueado" and p.recurso_esperado:
                self.txt_processos_bloqueados.insert(tk.END, f"Processo {p.id}: aguardando {p.recurso_esperado.nome}\n")
